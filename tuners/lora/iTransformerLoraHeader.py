...
class Model(iTransformer):
    def __init__(self, configs):
        ...
        if 'long_term_forecast' in self.task_name or self.task_name == 'short_term_forecast':
            ...
            self.lora_projection = nn.ModuleList([
                nn.Linear(configs.d_model, configs.pred_len, bias=True) for _ in range(self.num_loras)
            ])
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, **kwargs):
        ...
        dec_out = self.lora_projection[tuner_index](enc_out)
