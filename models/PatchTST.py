...
class FlattenHead(nn.Module):
    ...
    def forward(self, x, **kwargs):  # x: [bs x nvars x d_model x patch_num]
        x = self.flatten(x)
        x = self.linear(x) if isinstance(self.linear, nn.Linear) else self.linear(x, **kwargs)
        x = self.dropout(x)
        return x
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, **kwargs):
        ...
        enc_out, attns = self.encoder(enc_out, **kwargs)
        ...
        # Decoder
        dec_out = self.head(enc_out, **kwargs)  # z: [bs x nvars x target_window]
