...
def emd_1d_batched_align_t(preds, targets, idxi, idxj, invert=False, dist_scale=0.1, eps=1e-4, device='cpu'):
    ...
    target = torch.zeros_like(targets)
    pred = torch.zeros_like(preds)
    ...
    # time = 0.7
    seq = torch.arange(T).reshape(-1, 1).float().repeat(1, S).to(device).requires_grad_(True)
...
def emd_loss_1d_batched_align_all(pred, target, dist_scale=0.1, eps=1e-4, device='cpu'):
    ...
    seq = torch.arange(T).reshape(-1, 1).float().repeat(1, B * D).to(device).requires_grad_(True)
    emd = ot.wasserstein_1d(seq, seq, pred.T, target.T, p=1, require_sort=False)
    emd = emd / (T - 1) * dist_scale
    return emd


def emd_2d_batched_align_t(preds, targets, idxi, idxj, invert=False, dist_scale=0.1, eps=1e-4, device='cpu'):
    B, TD = targets.shape
    ...
    # time = 0.7
    seq = torch.arange(TD).reshape(-1, 1).float().repeat(1, B).to(device).requires_grad_(True)
    emd = ot.wasserstein_1d(seq, seq, pred.T, target.T, p=1, require_sort=False)
    emd = emd / (TD - 1) * dist_scale
    return emd
...
def emd_loss_2d_batched_align_t(pred, target, dist_scale=0.1, eps=1e-4, device='cpu'):
    ...
    posi, posj = torch.where(target >= 0)
    emd_p = emd_2d_batched_align_t(pred, target, posi, posj, dist_scale=dist_scale, eps=eps, device=device)

    negi, negj = torch.where(target < 0)
    emd_n = emd_2d_batched_align_t(pred, target, negi, negj, invert=True, dist_scale=dist_scale, eps=eps, device=device)

    return emd_p + emd_n


def emd_2d_batched_align_h(preds, targets, dist_scale=0.1, eps=1e-4, device='cpu'):
    B, TD = targets.shape
    preds = prob_norm_batched(preds, eps)
    targets = prob_norm_batched(targets, eps)
