...
def get_pca_base(data, rank_ratio=1.0, pca_dim="all", reinit=0):
    N, T, D = data.shape
    ...
    if pca_dim == "all":
        ...
        pca.fit(data)
        base = pca.components_  # shape: [full_rank * rank_ratio, T * D]

    elif pca_dim == "T":
        pca_components, initializer = [], []
        for d in range(D):
            ...
            pca = PCA(n_components=n_components)
            pca.fit(chunk)
            pca_components.append(pca.components_)  # shape: [full_rank * rank_ratio, T]
        ...
        base = np.array(pca_components)  # shape: [D, full_rank * rank_ratio, T]
    elif pca_dim == "D":
        pca_components, initializer = [], []
        for t in range(T):
            ...
            pca = PCA(n_components=n_components)
            pca.fit(chunk)
            pca_components.append(pca.components_)  # shape: [full_rank * rank_ratio, D]
        ...
        base = np.array(pca_components)  # shape: [T, full_rank * rank_ratio, D]
    else:
...
def pca_torch(data, pca_dim, pca_components, initializer, device='cpu'):
    B, T, D = data.shape

    if pca_dim == "all":
        data = data.reshape(B, -1)  # reshape to B, TD

    reinit = (len(initializer) != 0)
    if reinit:
        mean, std = [torch.from_numpy(value).float().to(device) for value in initializer]  # shape: [T * D]
        data = (data - mean) / std

    pca_components = torch.from_numpy(pca_components).float().to(device)
    if pca_dim == "all":
        # pca_components shape: [rank, T * D]
        low_rank_data = torch.mm(data, pca_components.T)  # shape: [B, rank]
    elif pca_dim == "T":
        # pca_components shape: [D, full_rank, T]
        low_rank_data = torch.einsum('btd,drt->brd', data, pca_components)  # shape: [B, rank, D]
    elif pca_dim == "D":
        # pca_components shape: [T, full_rank, D]
        low_rank_data = torch.einsum('btd,trd->btr', data, pca_components)  # shape: [B, T, rank]
