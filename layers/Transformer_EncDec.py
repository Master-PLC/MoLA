    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None, **kwargs):
        ...
        if self.conv_layers is not None:
            for i, (attn_layer, conv_layer) in enumerate(zip(self.attn_layers, self.conv_layers)):
                ...
                x = conv_layer(x)
                attns.append(attn)
            x, attn = self.attn_layers[-1](x, tau=tau, delta=None, **kwargs)
            attns.append(attn)
        else:
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, tau=None, delta=None, **kwargs):
        for layer in self.layers:
            x = layer(x, cross, x_mask=x_mask, cross_mask=cross_mask, tau=tau, delta=delta, **kwargs)

        if self.norm is not None:
            x = self.norm(x)

        if self.projection is not None:
            x = self.projection(x) if isinstance(self.projection, nn.Linear) else self.projection(x, **kwargs)
        return x
