...
class Exp_Long_Term_Forecast(Exp_Basic):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.auxi_lambda:
                        ...
                        if self.args.auxi_mode == "fft":
                            ...
                            loss_auxi = pca_torch(outputs, self.args.pca_dim, train_data.pca_components, train_data.initializer, device=self.device) - \
                                pca_torch(batch_y, self.args.pca_dim, train_data.pca_components, train_data.initializer, device=self.device)

                        elif self.args.auxi_mode == "ot":
                            if self.args.ot_type == "emd1d_h":
                                loss_auxi = emd_loss_1d_batched_align_h(outputs, batch_y, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd1d_t":
                                loss_auxi = emd_loss_1d_batched_align_t(outputs, batch_y, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd1d_all":
                                loss_auxi = emd_loss_1d_batched_align_all(outputs, batch_y, dist_scale=self.args.dist_scale, device=self.device)

                            elif self.args.ot_type == "emd2d_h":
                                loss_auxi = emd_loss_2d_batched_align_h(outputs, batch_y, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd2d_t":
                                loss_auxi = emd_loss_2d_batched_align_t(outputs, batch_y, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd2d_all":
                                loss_auxi = emd_loss_2d_batched_align_all(outputs, batch_y, dist_scale=self.args.dist_scale, device=self.device)

                            elif self.args.ot_type == "emd1d_h_learn_proj":
                                outputs_proj = self.model.project(outputs)
                                batch_y_proj = self.model.project(batch_y)
                                loss_auxi = emd_loss_1d_batched_align_h(outputs_proj, batch_y_proj, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd1d_t_learn_proj":
                                outputs_proj = self.model.project(outputs)
                                batch_y_proj = self.model.project(batch_y)
                                loss_auxi = emd_loss_1d_batched_align_t(outputs_proj, batch_y_proj, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd1d_all_learn_proj":
                                outputs_proj = self.model.project(outputs)
                                batch_y_proj = self.model.project(batch_y)
                                loss_auxi = emd_loss_1d_batched_align_all(outputs_proj, batch_y_proj, dist_scale=self.args.dist_scale, device=self.device)

                            elif self.args.ot_type == "emd1d_h_pca_proj":
                                ...
                                loss_auxi = emd_loss_1d_batched_align_h(outputs_proj, batch_y_proj, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd1d_t_pca_proj":
                                n_feats, rank_ratio = self.args.c_out, self.args.rank_ratio
                                low_rank = int(n_feats * rank_ratio)
                                outputs_proj = torch.matmul(outputs, torch.pca_lowrank(outputs.reshape(-1, n_feats), low_rank)[-1])
                                batch_y_proj = torch.matmul(batch_y, torch.pca_lowrank(batch_y.reshape(-1, n_feats), low_rank)[-1])
                                loss_auxi = emd_loss_1d_batched_align_t(outputs_proj, batch_y_proj, dist_scale=self.args.dist_scale, device=self.device)
                            elif self.args.ot_type == "emd1d_all_pca_proj":
                                n_feats, rank_ratio = self.args.c_out, self.args.rank_ratio
                                low_rank = int(n_feats * rank_ratio)
                                outputs_proj = torch.matmul(outputs, torch.pca_lowrank(outputs.reshape(-1, n_feats), low_rank)[-1])
                                batch_y_proj = torch.matmul(batch_y, torch.pca_lowrank(batch_y.reshape(-1, n_feats), low_rank)[-1])
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            print('loading pretrain model successfully')
